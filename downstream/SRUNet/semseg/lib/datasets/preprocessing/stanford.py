    ...
    def convert_to_ply(cls, root_path, out_path):
        ...
        for txtfile in tqdm(txtfiles):
            ...
            if len(coords) == 0:
                ...
                # #         inds, collabels, _ = ME.utils.sparse_quantize(
                # #             coords,
                # #             feats,
                # #             labels,
                # #             return_index=True,
                ...
                # #         )
                #         xcoords, xfeats, collabels, inds  = ME.utils.sparse_quantize(
                #             coords,
                #             feats,
                #             labels,
                #             return_index=True,
                ...
                #             quantization_size=0.01  # 1cm
                #         )
                # #         print(X[0].size(),X[1].shape, X[2].shape,X[3].size())
                # #         print(xcoords, xfeats, collabels, inds)
                # #         print(coords[inds].shape, feats[inds].shape, collabels[:, None].shape)
                #         pointcloud = np.concatenate((coords[inds], feats[inds], collabels), axis=1)
                coords = np.concatenate(coords, 0)
                feats = np.concatenate(feats, 0)
                labels = np.concatenate(labels, 0)
                coords2, feats2, labels2, labels2_2 = ME.utils.sparse_quantize(
                    coords, feats, labels, return_index=True, ignore_label=255, quantization_size=0.01
                )
                print("Size origin: ", np.max(labels[labels2_2]), "Size new: ", np.max(labels2))
