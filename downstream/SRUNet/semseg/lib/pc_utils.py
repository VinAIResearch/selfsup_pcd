...
class Camera(object):
    ...
    def extrinsics2RT(extrinsics):
        ...
        T = np.copy(T)
        T = T.reshape(3, 1)
        R[0, :] *= -1.0  # Step 1 of Kyle
        T *= 100.0  # Convert from m to cm
        return R, T
    ...
    def backproject(
        ...
        ones = np.ones_like(A)
        grid = np.concatenate((A[:, :, np.newaxis], B[:, :, np.newaxis], ones[:, :, np.newaxis]), axis=2)
...
class PlyWriter(object):
    ...
    POINTCLOUD_DTYPE = [("x", "f4"), ("y", "f4"), ("z", "f4"), ("red", "u1"), ("green", "u1"), ("blue", "u1")]
    ...
    def write_vertex_only_ply(cls, vertices, filepath):
        ...
        pc_points = np.array([tuple(p) for p in np.concatenate((vertices, color), axis=1)], dtype=cls.POINTCLOUD_DTYPE)
    ...
    def write_ply_vert_color(cls, vertices, colors, filepath):
        # assume that points are N x 3 np array for vertex locations
        pc_points = np.array(
            [tuple(p) for p in np.concatenate((vertices, colors), axis=1)], dtype=cls.POINTCLOUD_DTYPE
        )
        cls.write_ply(pc_points, filepath)
