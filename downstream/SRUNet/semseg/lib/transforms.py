        ...
        if random.random() < 0.95:
            for curr_ax in self.horz_axes:
                if random.random() < 0.5:
                    #           print("Check lib/transform 1", type(coords[:, curr_ax]))
                    #           print("Check lib/transform 1", np.amax(coords[:, curr_ax]))
                    #           exit()
                    coord_max = np.max(coords[:, curr_ax])
                    #           print("Check lib/transform 2", coords.shape)
...
class cfl_collate_fn_factory:
    ...
    def __call__(self, list_data):
        ...
        batch_id = 0
        batch_num_points = 0
        #     print("Check lib/transform 262", coords.shape)
