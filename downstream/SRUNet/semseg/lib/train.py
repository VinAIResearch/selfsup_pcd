...
#         exit()
        if torch.isinf(torch.flatten(coords)).float().sum(0).item() > 0.0 or torch.isnan(torch.flatten(coords)).float().sum(0).item() > 0.0:
            print('lib/train coords error')
        if torch.isinf(torch.flatten(input)).float().sum(0).item() > 0.0 or torch.isnan(torch.flatten(input)).float().sum(0).item() > 0.0:
