...
import numpy as np
import torch
from lib.distributed_utils import all_gather_list, get_rank, get_world_size
from lib.solvers import initialize_optimizer, initialize_scheduler
from lib.test import test
from lib.utils import AverageMeter, Timer, checkpoint, get_prediction, get_torch_device, precision_at_one
...
def train(model, data_loader, val_data_loader, config, transform_data_fn=None):
    ...
    # Train the network
    logging.info(
        "===> Start training on {} GPUs, batch-size={}".format(
            get_world_size(), config.data.batch_size * get_world_size()
        )
    )
    best_val_miou, best_val_iter, curr_iter, epoch, is_training = 0, 0, 1, 1, True

    if config.train.check_point:
        checkpoint_fn = config.train.check_point + "/weights/weights.pth"
    ...
    while is_training:
        for iteration in range(len(data_loader) // config.optimizer.iter_size):
            ...
            for sub_iter in range(config.optimizer.iter_size):
                # Get training data
                data_timer.tic()
                #         print('check data iteration (lib/train) 1')
                coords, input, target = data_iter.next()
                #         print('check data iteration (lib/train) 2')
                #         exit()
                # For some networks, making the network invariant to even, odd coords is important
                coords[:, :3] += (torch.rand(3) * 100).type_as(coords)
                #         print(type(coords), type(input), coords.size(), input.size())
                #         exit()
                if (
                    torch.isinf(torch.flatten(coords)).float().sum(0).item() > 0.0
                    or torch.isnan(torch.flatten(coords)).float().sum(0).item() > 0.0
                ):
                    print("lib/train coords error")
                if (
                    torch.isinf(torch.flatten(input)).float().sum(0).item() > 0.0
                    or torch.isnan(torch.flatten(input)).float().sum(0).item() > 0.0
                ):
                    print("lib/train color error")

                # Preprocess input
                color = input[:, :3].int()
                if config.augmentation.normalize_color:
                    input[:, :3] = input[:, :3] / 255.0 - 0.5
            ...
            if curr_iter % config.train.stat_freq == 0 or curr_iter == 1:
                ...
                debug_str = "===> Epoch[{}]({}/{}): Loss {:.4f}\tLR: {}\t".format(
                    epoch, curr_iter, len(data_loader) // config.optimizer.iter_size, losses.avg, lrs
                )
