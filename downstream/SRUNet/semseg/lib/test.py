    ...
    if has_gt:
        ...
        debug_str += (
            ...
            "mIOU {mIOU:.3f} mAP {mAP:.3f} mAcc {mAcc:.3f}\n".format(
                loss=losses, top1=scores, mIOU=np.nanmean(ious), mAP=np.nanmean(ap_class), mAcc=np.nanmean(acc)
            )
        )
...
def test(model, data_loader, config, transform_data_fn=None, has_gt=True):
    ...
    with torch.no_grad():
        for iteration in range(max_iter):
            ...
            # curr_transformation = transformation[:16].numpy().reshape(4, 4)
            # xyz = np.array(coords[:,1:].numpy())
            # xyz = np.hstack((xyz, np.ones((xyz.shape[0], 1))))
            # orig_coords = (np.linalg.inv(curr_transformation) @ xyz.T).T
            # print('Undo voxelized: ',np.max(orig_coords), np.min(orig_coords))
            # Preprocess input
            iter_timer.tic()
            ...
            # dataset.test_a_pointcloud(pred)
            # print(pred.shape, orig_coords.shape)
            # pred_ori = np.hstack((orig_coords[:,:3], pred.view(-1,1).cpu().numpy()))
            # ptc_pred,ptc_gt = dataset.test_a_pointcloud(pred_ori,iteration)
            # pred = torch.from_numpy(ptc_pred).to(device)
            # target = torch.from_numpy(ptc_gt)
            # exit()
            ...
            if has_gt:
                if config.test.evaluate_original_pointcloud:
                    ...
                    output, pred, target = permute_pointcloud(
                        coords, pointcloud, transformation, dataset.label_map, output, pred
                    )
    ...
    logging.info(
        "Loss:  {:.4f},  Score:  {:.4f}, mAP:  {:.4f}, mIoU:  {:.4f}".format(
            losses.avg, scores.avg, np.nanmean(ap_class), np.nanmean(per_class_iu(hist)) * 100
        )
    )
