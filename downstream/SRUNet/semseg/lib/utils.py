...
import numpy as np
import torch
from lib.distributed_utils import get_rank, get_world_size
...
def load_state_with_same_shape_no_conv0(model, weights):
    ...
    filtered_weights = {
        ...
        if k in model_state and v.size() == model_state[k].size() and "conv0p1s1" not in k
    }
    logging.info("Loading weights:" + ", ".join(filtered_weights.keys()))
    return filtered_weights
...
def load_state_with_same_shape_no_bn_stats(model, weights):
    ...
    filtered_weights = {
        k: v
        for k, v in weights.items()
        if k in model_state and v.size() == model_state[k].size() and "running" not in k and "tracked" not in k
    }
    logging.info("Loading weights:" + ", ".join(filtered_weights.keys()))
    return filtered_weights
...
def precision_at_one(pred, target, ignore_label=255):
    ...
    #   try:
    #   correct = correct[target != ignore_label]  # Error when ME version 0.5
    #   mask = torch.where(target != ignore_label, True, False) ##
    mask = target != ignore_label
    correct = torch.where(target != ignore_label, correct, False)
    #   except Exception as e:
    #     print(str(e))
    #     mask = torch.where(target != ignore_label, True, False)
    #     print('Check Lib/utils ', target.size(), correct.size(), pred.size(), mask.size())
    #     correct = correct[target != ignore_label]
    #     exit()
    correct = correct.view(-1)
    if correct.nelement():
        #     return correct.float().sum(0).mul(100.0 / correct.size(0)).item()
        return correct.float().sum(0).mul(100.0 / mask.float().sum()).item()
    else:
        return float("nan")
...
class ExpTimer(Timer):
    ...
    def toc(self):
        self.diff = time.time() - self.start_time
        self.average_time = self.alpha * self.diff + (1 - self.alpha) * self.average_time
        return self.average_time
