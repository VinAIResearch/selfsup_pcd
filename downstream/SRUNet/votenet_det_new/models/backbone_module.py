...
from models.backbone.pointnet2.pointnet2_modules import PointnetFPModule, PointnetSAModuleVotes
...
class SparseConvBackbone(nn.Module):
    def __init__(self, input_feature_dim=3, output_feature_dim=256, num_seed=1024, model="Res16UNet34C", config=None):
        ...
        self.net = load_model(model)(input_feature_dim, output_feature_dim, config)
        self.num_seed = num_seed

    def forward(self, points, coords, feats, inds, end_points=None):
        inputs = ME.SparseTensor(feats.cpu(), coordinates=coords.cpu().int(), device=coords.device)
        ...
        for b in range(bsz):
            sampled_id = (
                furthest_point_sample(points[voxel_ids[batch_ids == b]].unsqueeze(0), self.num_seed).squeeze(0).long()
            )
