...
net_arg.add_argument(
    "--weights_for_inner_model", type=str2bool, default=False, help="Weights for model inside a wrapper"
)
...
data_arg.add_argument("--num_workers", type=int, default=1, help="num workers for train/test dataloader")
...
data_arg.add_argument("--partial_crop", type=float, default=0.0)
...
data_arg.add_argument("--synthia_camera_path", type=str, default="/home/chrischoy/datasets/Synthia/%s/CameraParams/")
data_arg.add_argument("--synthia_camera_intrinsic_file", type=str, default="intrinsics.txt")
data_arg.add_argument("--synthia_camera_extrinsics_file", type=str, default="Stereo_Right/Omni_F/%s.txt")
...
train_arg.add_argument(
    "--resume_optimizer", default=True, type=str2bool, help="Use checkpoint optimizer states when resume training"
)
train_arg.add_argument("--eval_upsample", type=str2bool, default=False)
train_arg.add_argument(
    "--lenient_weight_loading", type=str2bool, default=False, help="Weights with the same size will be loaded"
)

# Distributed Training configurations
ddp_arg = add_argument_group("Distributed")
ddp_arg.add_argument(
...
ddp_arg.add_argument("--distributed-rank", default=0, type=int, help="rank of the current worker")
ddp_arg.add_argument("--distributed-backend", default="nccl", type=str, help="distributed backend")
ddp_arg.add_argument(
    "--distributed-init-method",
    default=None,
    type=str,
    help="typically tcp://hostname:port that will be used to " "establish initial connetion",
)
ddp_arg.add_argument(
    "--distributed-port", default=-1, type=int, help="port number (not required if using --distributed-init-method)"
)
ddp_arg.add_argument(
    "--device-id", "--local_rank", default=0, type=int, help="which GPU to use (usually configured automatically)"
)
ddp_arg.add_argument(
...
ddp_arg.add_argument(
    "--ddp-backend", default="c10d", type=str, choices=["c10d", "no_c10d"], help="DistributedDataParallel backend"
)
ddp_arg.add_argument("--bucket-cap-mb", default=25, type=int, metavar="MB", help="bucket size for reduction")
