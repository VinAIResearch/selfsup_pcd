...
from models.backbone.sparseconv.models.model import HighDimensionalModel
from models.backbone.sparseconv.models.modules.common import convert_region_type
from models.backbone.sparseconv.models.wrapper import Wrapper
...
class MeanField(HighDimensionalModel):
    ...
    # initialize_coords
    def __init__(self, nchannels, spatial_sigma, chromatic_sigma, meanfield_iterations, is_temporal, config, **kwargs):
    ...
    def initialize_coords(self, model, in_coords, in_color):
        if torch.prod(convert_to_int_tensor(model.OUT_PIXEL_DIST, model.D)) != 1:
            ...
            self.in_mapping = torch.sparse.FloatTensor(
                ...
                torch.ones(in_cols.size(0)),
                torch.Size((self.n_rows, in_cols.size(0))),
            )
            ...
            self.out_mapping = torch.sparse.FloatTensor(
                ...
                torch.ones(out_cols.size(0)),
                torch.Size((out_cols.size(0), self.n_rows)),
            )
