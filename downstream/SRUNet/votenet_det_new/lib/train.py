...
def evaluate_one_epoch(net, train_dataloader, test_dataloader, config, epoch_cnt, CONFIG_DICT, writer):
    ...
    for key in sorted(stat_dict.keys()):
        writer.add_scalar(
            ...
            stat_dict[key] / float(batch_idx + 1),
    ...
    writer.add_scalar(
        "validation/mAP@0.5", metrics_dict["mAP"], (epoch_cnt + 1) * len(train_dataloader) * config.data.batch_size
    )
...
def train(net, train_dataloader, test_dataloader, dataset_config, config):
    ...
    optimizer = optim.Adam(
        net.parameters(), lr=config.optimizer.learning_rate, weight_decay=config.optimizer.weight_decay
    )
