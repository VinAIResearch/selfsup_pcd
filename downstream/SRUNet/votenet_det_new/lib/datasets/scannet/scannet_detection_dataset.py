...
class ScannetDetectionDataset(Dataset):
    def __init__(
        ...
        if split_set in ["train", "val", "test"]:
            split_filenames = os.path.join(ROOT_DIR, "scannet/meta_data", "scannetv2_{}.txt".format(split_set))
            with open(split_filenames, "r") as f:
                self.scan_names = f.read().splitlines()
            # remove unavailiable scans
            num_scans = len(self.scan_names)
            self.scan_names = [sname for sname in self.scan_names if sname in all_scan_names]
            self.scan_names = self.scan_names[: int(len(self.scan_names) * data_ratio)]
    ...
    def __getitem__(self, idx):
        ...
        point_cloud, choices = pc_util.random_sampling(
            point_cloud, self.num_points, return_choices=True, fixseed=self.test_phase
        )
...
def viz_obb(pc, label, mask, angle_classes, angle_residuals, size_classes, size_residuals, name=""):
...
if __name__ == "__main__":
    ...
    for i_example in range(4):
        ...
        viz_votes(example["point_clouds"], example["vote_label"], example["vote_label_mask"], name=i_example)
        viz_obb(
