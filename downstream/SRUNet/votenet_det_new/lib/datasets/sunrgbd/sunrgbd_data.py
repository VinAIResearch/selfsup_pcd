...
def data_viz(data_dir, dump_dir=os.path.join(BASE_DIR, "data_viz_dump")):
    ...
    for idx in range(len(sunrgbd)):
        ...
        if len(oriented_boxes) > 0:
            ...
            pc_util.write_oriented_bbox(oriented_boxes, os.path.join(dump_dir, "obbs.ply"))
        else:
            print("-" * 30)
            continue
...
def extract_sunrgbd_data(
    ...
    for data_idx in data_idx_list:
        ...
        if skip_empty_scene and (
            len(objects) == 0 or len([obj for obj in objects if obj.classname in type_whitelist]) == 0
        ):
            continue
        ...
        np.savez_compressed(os.path.join(output_folder, "%06d_pc.npz" % (data_idx)), pc=pc_upright_depth_subsampled)
            ...
            for obj in objects:
                ...
                try:
                    ...
                    for i in range(len(sparse_inds)):
                        j = sparse_inds[i]
                        point_votes[j, int(point_vote_idx[j] * 3 + 1) : int((point_vote_idx[j] + 1) * 3 + 1)] = votes[
                            i, :
                        ]
...
def get_box3d_dim_statistics(idx_filename, type_whitelist=DEFAULT_TYPE_WHITELIST, save_path=None):
    ...
    for class_type in sorted(set(type_list)):
        ...
        print(
            "'%s': np.array([%f,%f,%f])," % (class_type, median_box3d[0] * 2, median_box3d[1] * 2, median_box3d[2] * 2)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--compute_median_size", action="store_true", help="Compute median 3D bounding box sizes for each class."
    )
