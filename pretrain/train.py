...
if __name__ == '__main__':
    ...
    # load data
    train_data = ModelNetSSLDataset(root=args.dataset, npoints=args.num_points, data_augmentation=False, num_views=args.num_views, num_point_contrast= args.num_point_contrast, fps = args.fps, pre_fix=args.pre_fix)
    ...
    if args.use_sgd:
        print("Use SGD")
        optimizer = optim.SGD(list(net2d.convup.parameters())+list(net3d.parameters()), lr=args.lr*100, momentum=args.momentum, weight_decay=1e-4)
    else:
        print("Use Adam")
    ...
    for epoch in range(1, args.nepoch+1):
        ...
        for data in tqdm(train_loader, total=len(train_loader)):
            ...
            loss_point=0.0
            for id in range(B):
                loss_point += contrastive_loss(F.normalize(point_feat3d[id][:,pix2point[id][:,3]].transpose(1,0), dim=-1), \
                                        F.normalize(pixel_feat2d[pix2point[id][:,0],:, pix2point[id][:,1],pix2point[id][:,2]], dim=-1), args.temperature)            
            loss_point /= B
        ...
        if args.scheduler == 'cos':
            ...
            if optimizer.param_groups[0]['lr'] < 1e-5:
                for param_group in optimizer.param_groups:
                    param_group['lr'] = 1e-5
        print('Epoch [%s]: loss: %s, loss point: %s, loss global: %s'%(epoch, total_loss/total_seen, total_loss_point/total_seen, total_loss_global/total_seen))
        if epoch%100==0:
            if args.model=='pointnet':
